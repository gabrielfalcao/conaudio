...
from conaudio.framework.http import JSONResource, json_response
from conaudio.api.models import User
from conaudio.api.roles import PublicRole
...
def get_authenticated_user(given_role_names=None):
    ...
    token = request.headers.get('X-ConAudio-Token', '')
    if not token:
        return None, forbidden('Missing X-ConAudio-Token')

    user = User.from_token(token)
    if not user:
        return None, forbidden('Invalid X-ConAudio-Token')
